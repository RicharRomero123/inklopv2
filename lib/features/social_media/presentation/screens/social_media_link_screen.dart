...
class _SocialMediaLinkScreenState extends State<SocialMediaLinkScreen> {
  ...
  void _openUrlModal(String platform) {
    showModalBottomSheet(
      ...
      builder: (_) => UrlInputModal(
        ...
        onConfirm: (url) async {
          final newAcc =
          await _apiService.linkAccount(platform, url, widget.accessToken);
          if (newAcc != null) {
  ...
  void _openVerificationModal(SocialMediaAccount account) {
    showModalBottomSheet(
      ...
      builder: (_) => VerificationModal(
        ...
        onVerify: () async {
          final success =
          await _apiService.verifyAccount(account.id, widget.accessToken);
          if (success && mounted) {
            Navigator.pop(context);    // cierra modal
            await _fetchAccounts();   // refresca lista → activa canContinue
            return true;
          }
          return false;
        },
  ...
  Widget build(BuildContext context) {
    final bool canContinue = _accounts.any((a) => a.isVerified == true);

    return PopScope(
      canPop: false, // bloquea gesto de back y botón físico
      child: Scaffold(
        backgroundColor: Colors.white,
        appBar: AppBar(
          backgroundColor: Colors.white,
          elevation: 0,
          automaticallyImplyLeading: false, // sin back button
        ),
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Botón Continuar — se activa cuando hay al menos 1 cuenta verificada
              SizedBox(
                ...
                child: FilledButton(
                  onPressed: canContinue
                    ...
                    MaterialPageRoute(
                      builder: (_) =>
                          MainScreen(accessToken: widget.accessToken),
                    ),
                  )
                      : null,
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () async {
              await _apiService.deleteAccount(acc.id, widget.accessToken);
              _fetchAccounts();
            },
            constraints: const BoxConstraints(),
            padding: const EdgeInsets.all(8),
